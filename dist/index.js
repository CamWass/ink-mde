...
import { SelectionRange, EditorSelection, Compartment, RangeSetBuilder, EditorState } from '@codemirror/state';
...
const lazyResolvers = [
  ...
  async ([state], compartment) => {
    ...
    if (options.lists || options.interface.lists) {
      const { lists } = await import('./lists-CgPfpDW-.js');
      let bullet = true;
      let number = true;
      let task = true;
...
export { appearanceTypes, ink as default, defineConfig, defineOptions, definePlugin, filterPlugins as f, hydrate, ink, inkPlugin, partitionPlugins as p, plugin, pluginTypes, render, renderToString, solidPrepareForHydration, wrap };
