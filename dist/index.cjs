...
const createState = ([state$1, setState]) => {
  const { selections } = state$1().options;
...
const Details = () => {
  ...
  return web.ssr(_tmpl$4$2, web.ssrHydrationKey(), web.escape(web.createComponent(solidJs.Show, {
    ...
    get children() {
      return web.ssr(_tmpl$$7, web.ssrHydrationKey(), web.escape(toHuman(state().doc)));
    }
...
const Button = props => {
  return web.ssr(_tmpl$$4, web.ssrHydrationKey(), web.escape(props.children));
};
...
const Styles = () => {
  ...
  return web.ssr(_tmpl$$2, web.ssrHydrationKey(), `.ink {\n  ${vars().join('\n  ')}\n}\n${styles}` || " ");
};
...
const Root = props => {
  ...
  })), "<!--/-->", web.ssr(_tmpl$$1, web.escape(web.createComponent(Editor, {
    get target() {
      return props.target;
    }
  }))), "<!--$-->", web.escape(web.createComponent(solidJs.Show, {
...
const wrap = (textarea, options = {}) => {
  const replacement = web.ssr(_tmpl$, web.ssrHydrationKey());
...
const getVals = (state, { from, to, type }) => {
  ...
  for (const index of Array(indentLevel).keys()) {
    ...
    const spacerDec = view.Decoration.replace({ widget: spacerWidget() }).range(from2, to2);
  ...
  const decorate = (state) => {
    ...
    language.syntaxTree(state).iterate({
      enter: (node) => {
        ...
        const lineDec = view.Decoration.line({
          attributes: {
        ...
        const dotDec = view.Decoration.replace({
          widget: dotWidget()
    ...
    return [view.Decoration.set(decorationRanges, true), view.Decoration.set(atomicRanges, true)];
  };
  ...
  const decorate = (state) => {
    ...
    language.syntaxTree(state).iterate({
      enter: (node) => {
        ...
        decorationRanges.push(...spacerDecorations);
        atomicRanges.push(...spacerDecorations);
        const taskDec = view.Decoration.replace({
...
const search = () => {
  return [search$2.search({
    top: true,
    createPanel: view$1 => {
      let query = search$2.getSearchQuery(view$1.state);
      ...
      const handleKeyDown = event => {
        if (view.runScopeHandlers(view$1, event, 'search-panel')) return event.preventDefault();
        if (event.code === 'Enter') {
          event.preventDefault();
          if (event.shiftKey) {
            search$2.findPrevious(view$1);
          } else {
            search$2.findNext(view$1);
          }
        }
      };
      const updateSearch = event => {
        ...
        query = new search$2.SearchQuery({
          search: value
        });
        view$1.dispatch({
          effects: search$2.setSearchQuery.of(query)
        });
      };
